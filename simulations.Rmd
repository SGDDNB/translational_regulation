...
get_DTG=function(counts_ribo, counts_rna, bcoeff, bgenes, te_genes, num_samples, 
    ...
    res_all <- matrix(nrow=nrow(res),ncol=7)
    res_all[,1] = res$padj
    ...
    res.riborex.edgeR <- riborex(sim_rna,sim_ribo, rnacond, rnacond,engine="edgeR")
    length(which(res.riborex.edgeR$table$FDR < 0.05))
    res_all[,3] = res.riborex.edgeR$table$FDR[order(as.numeric(rownames(res.riborex.edgeR$table)))]
    res.riborex.edgeRD <- riborex(sim_rna,sim_ribo, rnacond, rnacond,engine="edgeRD")
    length(which(res.riborex.edgeRD$table$FDR < 0.05))
    res_all[,4] = res.riborex.edgeRD$table$FDR[order(as.numeric(rownames(res.riborex.edgeRD$table)))]
    res.riborex.voom <- riborex(sim_rna,sim_ribo, rnacond, rnacond,engine="Voom")
    length(which(res.riborex.voom$adj.P.Val < 0.05))
    res_all[,5] = res.riborex.voom$adj.P.Val[order(as.numeric(rownames(res.riborex.voom)))]
    
    if(all_methods == TRUE){
      ### Running Xtail
      xtail <- xtail(sim_rna,sim_ribo,rnacond)
      length(which(xtail$resultsTable$pvalue.adjust < 0.05))
      res_all[,6] = xtail$resultsTable$pvalue.adjust
...
plot_ss=function(res_all,labels,dTE_genes,bcoeff,bgenes){
  ...
  plot(FDR_cutoffs,rep(0,length(FDR_cutoffs)),
    ylim=c(0,1),col="white",type="l",xlab="FDR cutoff",ylab="Sensitivity and Specificity")
  ...
  legend("topright",c("DESeq2","Riborex: DESeq2","Riborex: edgeR",
    "Riborex: edgeRD","Riborex: voom","Xtail","RiboDiff"),
...
for(i in 1:length(par_bcoeff)){
  results = get_DTG(counts_ribo, counts_rna, bcoeff=par_bcoeff[i], bgenes = 10, 
                    te_genes=10, num_samples=4, conditions=2, all_methods = TRUE) 
}
#}
#dev.off()
```